...
post_meal_data |>
  rename(
    P.Glucose.minus.5.OGTT = P.Glucose..5.OGTT,
    P.Glucose.minus.5.OGTT = P.Glucose..5.OGTT
  )
...
small_post_meal_data %>%
  ...
  summarize(n_distinct(BMI))

small_post_meal_data %>%
  filter(Group == "CTR" & BMI > 25) %>%
  summarize(n_distinct(glykemi), sum(glykemi), n = n())
```
## Modifying data
...
tidied_post_meal_data %>%
  summarise(
    sum_BMI = sum(BMI, na.rm = TRUE),
    distinct_BMI = n_distinct(BMI),
    n_BMI = n(),
    max_bmi = max(BMI),
    min_BMI = min(BMI)
  )
...
#| tbl-cap: "Mean values og age and BMI for each group"
...
#| tbl-cap: "Median values of AUC meassurements by age group and family history group"
post_meal_data |>
  # Task 2.
  filter(BMI < 30) |>
  # Task 3.
